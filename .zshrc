...
plugins=(z aliases fzf macos extract colored-man-pages copypath copyfile copybuffer history)
...
if [[ -o interactive && -z "$FASTFETCH_SHOWN" ]]; then
  ...
  fastfetch --kitty "$dotspath"/images/logo.png
fi

(
  images=("$dotspath"/images/logos/*.png(N.))
  logo=${images[$((RANDOM % ${#images[@]} + 1))]}
  ln -sfn "$logo" "$dotspath"/images/logo.png
) &!
